...
<head>
    ...
    <title>Modifier la Compétence - Debrief.me</title>
    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 ml-64 p-8">
            <header class="mb-10 flex items-center gap-4">
                <a href="{{ route('admin.competences.index') }}" class="p-3 bg-white/5 text-slate-400 rounded-2xl hover:text-white transition-all">
                    <i data-lucide="arrow-left"></i>
                </a>
                <div>
                    <h1 class="text-3xl font-extrabold">Modifier la Compétence</h1>
                    <p class="text-slate-400 mt-1">Mise à jour des informations de la compétence {{ $competence->code }}</p>
                </div>
            </header>

            <div class="max-w-2xl">
                <form action="{{ route('admin.competences.update', $competence->code) }}" method="POST" class="glass p-8 rounded-3xl space-y-6">
                    @csrf
                    @method('PUT')
                    
                    <div>
                        <label class="block text-sm font-bold text-slate-400 uppercase tracking-widest mb-2">Code de Compétence</label>
                        <input type="text" name="code" value="{{ old('code', $competence->code) }}" placeholder="Ex: C1" 
                               class="w-full bg-slate-900/50 border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-indigo-500 transition-all @error('code') border-rose-500/50 @enderror">
                        <p class="mt-2 text-xs text-slate-500 italic">Format requis: "C" suivi d'un ou plusieurs chiffres (C1, C2, etc.)</p>
                        @error('code')
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-400 uppercase tracking-widest mb-2">Libellé</label>
                        <input type="text" name="label" value="{{ old('label', $competence->label) }}" placeholder="Ex: Maquetter une application" 
                               class="w-full bg-slate-900/50 border border-white/10 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-indigo-500 transition-all @error('label') border-rose-500/50 @enderror">
                        @error('label')
                            <p class="mt-2 text-sm text-rose-400 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="pt-4">
                        <button type="submit" class="w-full bg-indigo-500 hover:bg-indigo-600 text-white py-4 rounded-2xl font-bold transition-all shadow-lg shadow-indigo-500/20 flex items-center justify-center gap-2">
                            <i data-lucide="refresh-cw"></i> Mettre à jour la Compétence
                        </button>
                    </div>
                </form>
            </div>
