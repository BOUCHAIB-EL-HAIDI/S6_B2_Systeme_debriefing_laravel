...
<head>
    ...
    <title>Ajouter une Compétence - Debrief.me</title>
    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 ml-64 p-8">
            <header class="mb-10 flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-extrabold">Ajouter une Compétence</h1>
                    <p class="text-slate-400 mt-1">Définissez une nouvelle compétence pédagogique</p>
                </div>
            </header>

            <div class="max-w-2xl">
                <form action="{{ route('admin.competences.store') }}" method="POST" class="glass p-8 rounded-3xl space-y-6">
                    @csrf
                    
                    <div>
                        <label class="block text-sm font-bold text-slate-400 uppercase tracking-widest mb-2">Code de Compétence</label>
                        <input type="text" name="code" value="{{ old('code') }}" placeholder="Ex: C1" 
                    ...
                    <div>
                        ...
                        <input type="text" name="label" value="{{ old('label') }}" placeholder="Ex: Maquetter une application" 
                    ...
                    <div class="pt-4">
                        <button type="submit" class="w-full bg-indigo-500 hover:bg-indigo-600 text-white py-4 rounded-2xl font-bold transition-all shadow-lg shadow-indigo-500/20 flex items-center justify-center gap-2">
                            <i data-lucide="save"></i> Enregistrer la Compétence
                        </button>
                    </div>
                </form>
            </div>
